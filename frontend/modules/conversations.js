...
export function renderConversations() {
  ...
  state.conversations.forEach((c) => {
    ...
    div.setAttribute("aria-label", `Select conversation: ${c.title || "New Chat"}`);
    div.className = `conversation-item flex items-center justify-between px-3 py-2 text-sm rounded-r-lg cursor-pointer transition-all group focus-visible:ring-2 focus-visible:ring-primary/50 outline-none ${
    ...
    const selectHandler = () => loadConversation(c.id);
    ...
    div.querySelector(".delete-btn").addEventListener("click", async (e) => {
      ...
      // No need to enable btn as list is re-rendered
    });
    div.querySelector(".export-btn").addEventListener("click", async (e) => {
      e.stopPropagation();
      const btn = e.currentTarget;
      btn.disabled = true;
      await exportConversation(c.id, c.title || "conversation");
      btn.disabled = false;
    });
