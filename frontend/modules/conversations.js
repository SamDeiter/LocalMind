  ...
  state.conversations.forEach((c) => {
    ...
    div.innerHTML = `
      ...
      <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
        <button class="export-btn p-1 text-outline hover:text-secondary rounded flex" title="Export" aria-label="Export conversation">
          <span class="material-symbols-outlined text-base">download</span>
        </button>
        <button class="delete-btn p-1 text-outline hover:text-error rounded flex" title="Delete" aria-label="Delete conversation">
