...
export function renderConversations() {
  ...
  state.conversations.forEach((c) => {
    ...
    div.className = `conversation-item flex items-center justify-between pr-2 text-sm rounded-r-lg transition-all group ${
      c.id === state.currentConvId
        ? "text-[#c0c1ff] bg-primary/10 border-l-2 border-[#6366f1]"
        : "text-[#8e9192] hover:bg-[#1c1b1b]"
    }`;
    div.innerHTML = `
      <button class="conv-title-btn flex-1 text-left truncate px-3 py-2 hover:text-[#e5e2e1] transition-colors" title="${escapeHtml(
        c.title || "New Chat"
      )}">
