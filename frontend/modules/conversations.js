  ...
  state.conversations.forEach((c) => {
    ...
    div.innerHTML = `
      ...
      <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
        <button class="export-btn p-1 text-outline hover:text-secondary rounded flex items-center" title="Export Chat" aria-label="Export Chat">
          <span class="material-symbols-outlined text-[18px]">download</span>
        </button>
        <button class="delete-btn p-1 text-outline hover:text-error rounded flex items-center" title="Delete Chat" aria-label="Delete Chat">
