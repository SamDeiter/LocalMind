  ...
  state.conversations.forEach((c) => {
    ...
    div.innerHTML = `
      ...
      <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
        <button class="export-btn p-0.5 text-outline hover:text-secondary rounded transition-colors" title="Export" aria-label="Export conversation">
          <span class="material-symbols-outlined text-[18px]">download</span>
        </button>
        <button class="delete-btn p-0.5 text-outline hover:text-error rounded transition-colors" title="Delete" aria-label="Delete conversation">
