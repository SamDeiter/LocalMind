...
export function renderConversations() {
  ...
  state.conversations.forEach((c) => {
    ...
    div.innerHTML = `
      <button class="flex-1 text-left truncate pr-2 hover:text-white transition-colors" title="${escapeHtml(c.title || "New Chat")}">
      ...
      <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
        <button class="export-btn p-1 text-outline hover:text-secondary rounded flex items-center" title="Export conversation" aria-label="Export conversation">
          <span class="material-symbols-outlined text-base">download</span>
        </button>
        <button class="delete-btn p-1 text-outline hover:text-error rounded flex items-center" title="Delete conversation" aria-label="Delete conversation">
          <span class="material-symbols-outlined text-base">delete</span>
        </button>
      </div>`;
    div.querySelector("button.flex-1").addEventListener("click", () => loadConversation(c.id));
