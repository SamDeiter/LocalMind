...
export function renderConversations() {
  ...
  state.conversations.forEach((c) => {
    ...
    div.className = `conversation-item group flex items-center justify-between rounded-r-lg transition-all ${
      isActive
    ...
    div.innerHTML = `
      <button class="flex-1 text-left px-3 py-2 text-sm truncate focus:outline-none" aria-label="Load conversation: ${escapeHtml(
        c.title || "New Chat"
      )}">
        ${escapeHtml(c.title || "New Chat")}
      </button>
      <div class="flex items-center gap-0.5 pr-1 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
