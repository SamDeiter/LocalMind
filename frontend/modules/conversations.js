...
export function renderConversations() {
  ...
  state.conversations.forEach((c) => {
    ...
    div.innerHTML = `
      <button class="flex-1 text-left truncate pr-2 py-1 outline-none focus:text-primary transition-colors ${
        c.id === state.currentConvId ? "text-[#c0c1ff]" : "text-[#8e9192] group-hover:text-[#e5e2e1]"
      }" aria-label="Load conversation: ${escapeHtml(c.title || "New Chat")}">
