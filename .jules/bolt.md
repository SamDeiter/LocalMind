...
## 2026-05-16 - Offloading Synchronous SQLite DB Operations
**Learning:** Performing synchronous SQLite queries directly in async FastAPI route handlers blocks the event loop. In multi-user or high-concurrency environments (like active chats or streaming SSE), blocking the event loop on database disk I/O significantly degrades latency and throughput for other concurrent requests.
**Action:** Always offload synchronous database read/write operations in route handlers to worker threads via `asyncio.to_thread` to ensure the event loop remains completely non-blocking.
