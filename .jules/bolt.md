...
## 2025-05-16 - Shared httpx.AsyncClient for Connection Pooling
**Learning:** Creating a new `httpx.AsyncClient` for every request is an anti-pattern in high-concurrency async apps. It causes overhead for TCP connection establishment and fails to reuse connections via keep-alive. Using a singleton client with `limits=httpx.Limits(max_connections=100, max_keepalive_connections=20)` significantly improves throughput and reduces latency.
**Action:** Always use a shared `httpx.AsyncClient` singleton (e.g. from `backend.utils.http_client`) and explicitly pass `timeout` for long-running requests like LLM streaming to override the default.
