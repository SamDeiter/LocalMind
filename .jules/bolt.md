...
## 2026-05-21 - SQL-side Telemetry Aggregation
**Learning:** Parsing JSON metrics in Python loops scales poorly (O(N) with high overhead). Shifting aggregation to SQLite using `json_extract()` and `SUM/COUNT` combined with a composite index on `(recorded_at, metric_type)` reduced latency by ~40-60% on large datasets.
**Action:** Use SQL-side aggregation for metrics stored in JSON columns when possible.
