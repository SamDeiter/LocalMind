...
# ── Synchronous Database Helpers for Thread Offloading ─────────────────
...
def _sync_get_messages(conv_id: str):
    db = _get_db()
    try:
        rows = db.execute(
...
def _sync_delete_conversation(conv_id: str):
    db = _get_db()
    try:
...
def _sync_get_conversation(conv_id: str):
    db = _get_db()
    try:
...
def _sync_update_system_prompt(conv_id: str, prompt: str):
    db = _get_db()
    try:
        db.execute(
...
def _sync_get_export_data(conv_id: str):
    db = _get_db()
    try:
        ...
        return dict(conv), [dict(m) for m in msgs]
    finally:
        db.close()
...
async def list_conversations():
    ...
    """
    conversations = await asyncio.to_thread(_sync_list_conversations)
...
async def get_conversation_messages(conv_id: str):
    ...
    """
    messages = await asyncio.to_thread(_sync_get_messages, conv_id)
...
async def delete_conversation(conv_id: str):
    ...
    """
    await asyncio.to_thread(_sync_delete_conversation, conv_id)
...
async def get_conversation(conv_id: str):
    ...
    """
    conv = await asyncio.to_thread(_sync_get_conversation, conv_id)
    if not conv:
...
async def update_system_prompt(conv_id: str, request: Request):
    ...
    await asyncio.to_thread(_sync_update_system_prompt, conv_id, prompt)
...
async def export_conversation(conv_id: str, format: str = "md"):
    ...
    """
    conv, msgs = await asyncio.to_thread(_sync_get_export_data, conv_id)
