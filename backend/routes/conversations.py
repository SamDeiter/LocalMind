...
async def list_conversations():
    ...
    """
    # ⚡ Bolt: Offload synchronous database operations to a worker thread
    ...
    rows = await asyncio.to_thread(sync_op)
...
async def get_conversation(conv_id: str):
    ...
    row = await asyncio.to_thread(sync_op)
    if not row:
...
async def export_conversation(conv_id: str, format: str = "md"):
    ...
    conv, msgs = await asyncio.to_thread(sync_op)
