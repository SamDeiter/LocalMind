...
SENSITIVE_FIELDS = ["smtp_pass", "api_key"]
...
def _preserve_secrets(incoming: dict, current: dict) -> dict:
    """Restore original secrets if the incoming data contains masked values."""
    processed = incoming.copy()
    for field in SENSITIVE_FIELDS:
        if processed.get(field) == "********":
            processed[field] = current.get(field, "")
    return processed
...
async def update_notification_settings(settings: dict):
    ...
    processed = _preserve_secrets(settings, current)
    notifications.save_settings(processed)
    return {"status": "ok", "settings": _mask_settings(processed)}
...
async def update_cloud_settings(settings: dict):
    ...
    processed = _preserve_secrets(settings, current)
    gemini_client.save_settings(processed)
    return {"status": "ok"}
