...
class LLMClient:
    ...
    async def _stream_ollama(
        ...
        logger.info(
            f"Ollama request: model={payload.get('model')}, messages={len(payload.get('messages',[]))}, keys={list(payload.keys())}"
        )

        client = get_async_client()
        for attempt in range(max_retries):
            try:
                # ⚡ Bolt: Use shared client with explicit timeout for LLM streaming
                    ...
                    async for line in response.aiter_lines():
                        ...
                        try:
                            ...
                            yield {
                                ...
                                "tool_calls": data.get("message", {}).get(
                                    "tool_calls", []
                                ),
                            }
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"Ollama attempt {attempt+1} failed: {e}. Retrying in {wait_time}s..."
                    )
                    await asyncio.sleep(wait_time)
                else:
                    logger.error(
                        f"Ollama stream failed after {max_retries} attempts: {e}"
                    )
                    yield {"error": str(e)}
    ...
    async def generate(
        ...
        **kwargs,
        ...
        client = get_async_client()
        try:
            # ⚡ Bolt: Use shared client with explicit timeout for LLM generation
