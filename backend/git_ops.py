...
def git_run(args: list[str]) -> str:
    ...
    try:
        ...
        # Remove "cmd /c" which is insecure and unnecessary when using subprocess.run with a list.
        full_args = ["git"] + args
...
async def run_tests(target_files: list[str] = None) -> tuple[bool, str]:
    ...
    try:
        ...
        full_args = [sys.executable, "-m", "pytest"] + test_targets + ["-q", "--tb=short", "-x"]
        result = subprocess.run(
            full_args,
...
def count_tests() -> int:
    ...
    try:
        # Security: Use list-based arguments and shell=False to prevent command injection.
        full_args = [sys.executable, "-m", "pytest", "tests/", "--collect-only", "-q"]
        result = subprocess.run(
            full_args,
