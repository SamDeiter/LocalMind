...
try:
    ...
    def safe_resolve(base_dir: Path | str, user_path: str | Path) -> Path:  # type: ignore[misc]
        ...
        if not candidate.is_relative_to(base):
            raise RuntimeError(f"Path {user_path!r} escapes jail {base!r}")
        return candidate
        ...
        for arg_name, arg_value in args.items():
            if isinstance(arg_value, str):
                ...
                if any(kw in lower_name for kw in ("path", "file", "dir", "folder", "dest", "src")):
                    try:
                        # Standardized argument order: (base_dir, user_path)
                        resolved = safe_resolve(job_dir, arg_value)
                        # safe_resolve already performs jail check, but we can be extra sure
