...
class AcademicResearcher:
    ...
    _last_arxiv_call: float = 0  # rate-limit tracker
    ...
    async def search_arxiv(self, query: str, max_results: int = 5, start: int = 0) -> list[dict]:
        ...
        Enforces a 3-second minimum gap between API calls.
        """
        import asyncio
        import time
        import feedparser

        # Rate-limit: 3s min between calls
        now = time.monotonic()
        gap = now - self._last_arxiv_call
        if gap < 3.0:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                # Handle rate-limiting: retry once after 5s
                if resp.status_code == 429:
                    logger.warning("arXiv rate-limited (429) — retrying after 5s")
                    await asyncio.sleep(5.0)
                    resp = await client.get(url, params=params)

                # arXiv overloaded
                if resp.status_code == 503:
                    logger.warning("arXiv overloaded (503) — returning empty")
                    return []
