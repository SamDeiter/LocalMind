...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        Produce an aggregated metrics summary using SQL aggregation.
        ...
        # This avoids fetching and parsing thousands of rows in Python.
        query = """
            SELECT
                ...
                COALESCE(SUM(json_extract(value_json, '$.latency_ms')) FILTER (WHERE metric_type = 'llm_call'), 0.0) as llm_latency_sum,
                COALESCE(SUM(json_extract(value_json, '$.cost_cents')) FILTER (WHERE metric_type = 'llm_call'), 0.0) as llm_cost_sum,
                ...
                COALESCE(SUM(json_extract(value_json, '$.duration_ms')) FILTER (WHERE metric_type = 'tool_call'), 0.0) as tool_duration_sum,
                ...
                COALESCE(SUM(json_extract(value_json, '$.duration_ms')) FILTER (WHERE metric_type = 'job_completion'), 0.0) as job_duration_sum,
                COALESCE(SUM(json_extract(value_json, '$.total_cost')) FILTER (WHERE metric_type = 'job_completion'), 0.0) as job_cost_sum
            FROM metrics
        """
        params = []
        if since:
        ...
        if not row:
            # Should not happen with aggregate query on empty table, but for safety:
            return {
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        # ⚡ Bolt: Offload failure rate and latency aggregation to SQL.
        # Avoids loading and parsing every metric JSON blob in Python.
        conn = _connect()
        try:
            ...
            if stats_row and stats_row["total"] > 0:
                ...
                # 2. P95 latency (fetch only the necessary value)
                ...
                if latency_row and latency_row["p95"] is not None:
                    p95 = latency_row["p95"]
