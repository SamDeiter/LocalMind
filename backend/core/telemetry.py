...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        Produce an aggregated metrics summary using database-side aggregation.
        ...
        conn = _connect()
        try:
            # ⚡ Bolt: Offload aggregation to SQLite using json_extract and FILTER clauses.
            ...
            query = """
                SELECT
                    ...
                    AVG(CAST(json_extract(value_json, '$.latency_ms') AS REAL)) FILTER (WHERE metric_type = 'llm_call') as llm_avg_latency,
                    ...
                    AVG(CAST(json_extract(value_json, '$.duration_ms') AS REAL)) FILTER (WHERE metric_type = 'tool_call') as tool_avg_duration,
                    ...
                    AVG(CAST(json_extract(value_json, '$.duration_ms') AS REAL)) FILTER (WHERE metric_type = 'job_completion') as job_avg_duration,
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            # ⚡ Bolt: Use SQL to calculate failure rate and P95 latency directly.
            # This avoids fetching all job metrics and performing sorting in Python.
            ...
            fail_query = f"""
                ...
                FROM metrics {where}
            """
            f_row = conn.execute(fail_query, params).fetchone()
            ...
            if f_row and f_row["total"] > 0:
                ...
                # 2. P95 latency using window function
                p95_query = f"""
                    WITH Latencies AS (
                        SELECT
                            CAST(json_extract(value_json, '$.duration_ms') AS REAL) as lat,
                    ...
                    LIMIT 1
                """
                p_row = conn.execute(p95_query, params).fetchone()
                if p_row:
                    p95 = p_row["lat"]
