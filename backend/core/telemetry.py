...
def init_telemetry_schema() -> None:
    ...
    conn.executescript("""
        ...
        -- ⚡ Bolt: Composite index for fast aggregation and threshold checking
...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        # This avoids loading thousands of JSON blobs into Python memory.
        sql = """
            SELECT
                -- LLM Calls
        ...
        if not r:
            # Should not happen with an aggregate query without GROUP BY, but safety first
            return {}
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            # ── Job metrics (Failure Rate & Latency) ─────────────────────
            # ⚡ Bolt: Extract only specific fields from JSON via SQL to minimize data transfer.
            # While P95 needs Python-side sorting (SQLite lacks native P95), failure rate
            # can be partially computed in SQL.
