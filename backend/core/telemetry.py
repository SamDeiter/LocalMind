...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        # ⚡ Bolt: Optimize by offloading JSON parsing and aggregation to SQL
        # This avoids fetching all rows into Python memory and repeated json.loads() calls.
        conn = _connect()
        try:
            query = """
                SELECT
                    ...
                    COUNT(*) FILTER (WHERE metric_type = 'tool_call' AND json_extract(value_json, '$.status') = 'ok')
                        as tool_ok,
                    COUNT(*) FILTER (WHERE metric_type = 'tool_call'
                        AND json_extract(value_json, '$.status') IS NOT 'ok')
                        as tool_error,
        ...
        llm_count = res.get("llm_count") or 0
        tool_count = res.get("tool_count") or 0
        job_count = res.get("job_count") or 0

        return {
            "llm_calls": {
                "total": llm_count,
                "tokens_in": res.get("llm_tokens_in") or 0,
                "tokens_out": res.get("llm_tokens_out") or 0,
                "avg_latency_ms": round((res.get("llm_latency_sum") or 0) / llm_count, 1) if llm_count else 0,
                "total_cost_cents": round(res.get("llm_cost_sum") or 0, 4),
            },
            "tool_calls": {
                "total": tool_count,
                "success": res.get("tool_ok") or 0,
                "error": res.get("tool_error") or 0,
                "avg_duration_ms": round((res.get("tool_duration_sum") or 0) / tool_count, 1) if tool_count else 0,
            },
            "job_completions": {
                "total": job_count,
                "completed": res.get("job_ok") or 0,
                "failed": res.get("job_fail") or 0,
                "avg_duration_ms": round((res.get("job_duration_sum") or 0) / job_count, 1) if job_count else 0,
                "total_cost_cents": round(res.get("job_cost_sum") or 0, 4),
            },
        }
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        # ⚡ Bolt: Optimize by calculating failure rate and P95 latency in SQL.
        # This avoids fetching all job rows and sorting them in Python memory.
        conn = _connect()
        try:
            # ── Job failure rate & P95 latency ──────────────────────────
            where_since = "AND recorded_at >= ?" if since else ""
            ...
            fail_query = f"""
                SELECT
                    COUNT(*) as total,
                    COUNT(*) FILTER (WHERE json_extract(value_json, '$.status') IS NOT 'completed') as failed
                FROM metrics
                WHERE metric_type = 'job_completion' {where_since}
            """
            row = conn.execute(fail_query, params).fetchone()
            ...
            # 2. P95 Latency using a window function
            p95_query = f"""
                WITH latencies AS (
                    SELECT
                        CAST(json_extract(value_json, '$.duration_ms') AS REAL) as duration_ms,
                        ROW_NUMBER() OVER (ORDER BY CAST(json_extract(value_json, '$.duration_ms') AS REAL)) as row_num,
                ...
                LIMIT 1
            """
            row = conn.execute(p95_query, params).fetchone()
            if row:
                p95 = row["duration_ms"]
