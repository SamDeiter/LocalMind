...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        # ⚡ Bolt: Using SQL-side aggregation with json_extract and FILTER to avoid
        # loading thousands of rows into Python memory. This is O(1) in terms of
        # data transfer regardless of metric count.
        query = """
            SELECT
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # ⚡ Bolt: Fetching only specific JSON fields via SQL to avoid parsing
            # massive blobs in Python. We fetch status for rate and duration_ms
            # for P95.
