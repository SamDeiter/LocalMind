...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        try:
            ...
            # ⚡ Bolt: Offload aggregation to SQLite using json_extract and FILTER.
            # This is significantly faster for large datasets than a Python loop.
            query = f"""
                SELECT
                    -- LLM calls
                    COALESCE(COUNT(*) FILTER (WHERE metric_type = 'llm_call'), 0)
                        as llm_count,
                    COALESCE(SUM(json_extract(value_json, '$.tokens_in')) FILTER (WHERE metric_type = 'llm_call'), 0)
                        as llm_tokens_in,
                    COALESCE(SUM(json_extract(value_json, '$.tokens_out')) FILTER (WHERE metric_type = 'llm_call'), 0)
                        as llm_tokens_out,
                    COALESCE(SUM(json_extract(value_json, '$.latency_ms')) FILTER (WHERE metric_type = 'llm_call'), 0)
                        as llm_latency_sum,
                    COALESCE(SUM(json_extract(value_json, '$.cost_cents')) FILTER (WHERE metric_type = 'llm_call'), 0)
                        as llm_cost_sum,

                    -- Tool calls
                    COALESCE(COUNT(*) FILTER (WHERE metric_type = 'tool_call'), 0)
                        as tool_count,
                    COALESCE(COUNT(*) FILTER (WHERE metric_type = 'tool_call'
                        AND json_extract(value_json, '$.status') = 'ok'), 0) as tool_ok,
                    COALESCE(COUNT(*) FILTER (WHERE metric_type = 'tool_call'
                        AND json_extract(value_json, '$.status') != 'ok'), 0) as tool_error,
                    COALESCE(SUM(json_extract(value_json, '$.duration_ms')) FILTER (WHERE metric_type = 'tool_call'), 0)
                        as tool_duration_sum,

                    -- Job completions
                    COALESCE(COUNT(*) FILTER (WHERE metric_type = 'job_completion'), 0)
                        as job_count,
                    COALESCE(COUNT(*) FILTER (WHERE metric_type = 'job_completion'
                        AND json_extract(value_json, '$.status') = 'completed'), 0) as job_ok,
                    COALESCE(COUNT(*) FILTER (WHERE metric_type = 'job_completion'
                        AND json_extract(value_json, '$.status') != 'completed'), 0) as job_fail,
                    COALESCE(SUM(json_extract(value_json, '$.duration_ms'))
                        FILTER (WHERE metric_type = 'job_completion'), 0)
                        as job_duration_sum,
                    COALESCE(SUM(json_extract(value_json, '$.total_cost'))
                        FILTER (WHERE metric_type = 'job_completion'), 0)
                        as job_cost_sum
                FROM metrics
                {where}
        ...
        return {
            "llm_calls": {
                ...
                "avg_latency_ms": round(row["llm_latency_sum"] / row["llm_count"], 1) if row["llm_count"] else 0,
            ...
            "tool_calls": {
                ...
                "avg_duration_ms": round(row["tool_duration_sum"] / row["tool_count"], 1) if row["tool_count"] else 0,
            },
            "job_completions": {
                "total": row["job_count"],
                "completed": row["job_ok"],
                "failed": row["job_fail"],
                "avg_duration_ms": round(row["job_duration_sum"] / row["job_count"], 1) if row["job_count"] else 0,
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            # ⚡ Bolt: Use SQL aggregation and subqueries to calculate failure rates and P95 latency.
            # This avoids loading and sorting thousands of records in Python.
            ...
            if stats and stats["total"] > 0:
                ...
                # ── P95 latency ─────────────────────────────────────────
                # Calculate p95 offset: max(0, floor(total * 0.95) - 1)
                p95_offset = max(0, int(total * 0.95) - 1)
                p95_query = f"""
                    SELECT json_extract(value_json, '$.duration_ms') as p95
                    FROM metrics {where}
                    ORDER BY CAST(json_extract(value_json, '$.duration_ms') AS REAL) ASC
                    LIMIT 1 OFFSET {p95_offset}
                """
                p95_row = conn.execute(p95_query, params).fetchone()
