...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        # ⚡ Bolt: Use SQL-side aggregation to avoid fetching all rows into memory
        # and parsing JSON for every row in Python. This is significantly faster
        # for large datasets (50,000+ records).
        sql = """
            SELECT
                -- LLM Calls
                COUNT(*) FILTER (WHERE metric_type = 'llm_call') as llm_count,
                TOTAL(CAST(json_extract(value_json, '$.tokens_in') AS INTEGER)) FILTER (WHERE metric_type = 'llm_call') as llm_tokens_in,
                TOTAL(CAST(json_extract(value_json, '$.tokens_out') AS INTEGER)) FILTER (WHERE metric_type = 'llm_call') as llm_tokens_out,
                AVG(CAST(json_extract(value_json, '$.latency_ms') AS REAL)) FILTER (WHERE metric_type = 'llm_call') as llm_avg_latency,
                TOTAL(CAST(json_extract(value_json, '$.cost_cents') AS REAL)) FILTER (WHERE metric_type = 'llm_call') as llm_cost_sum,
                ...
                TOTAL(CAST(json_extract(value_json, '$.total_cost') AS REAL)) FILTER (WHERE metric_type = 'job_completion') as job_cost_sum
            FROM metrics
        """
        params = []
        if since:
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # ⚡ Bolt: SQL-side specific value extraction to avoid loading full
            # JSON blobs into memory. We fetch status and duration for all
            # relevant rows to compute both the failure rate and P95 latency.
            sql = """
                SELECT
            ...
            if rows:
                ...
                # ── P95 latency ─────────────────────────────────────────
                latencies = sorted(
                    float(r["duration"] or 0) for r in rows if r["duration"] is not None
                )
                if latencies:
