...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        Produce an aggregated metrics summary using SQL-native JSON functions.
        ...
        # ⚡ Bolt: Offload JSON parsing and aggregation to the database.
        # This is significantly faster than loading all rows into memory and
        # parsing them in Python.
        query = f"""
            SELECT
                metric_type,
                COUNT(*) as count,
                ...
                COUNT(*) FILTER (
                    WHERE json_extract(value_json, '$.status') IS NOT 'ok'
                    AND metric_type = 'tool_call'
                ) as tool_error,
                COUNT(*) FILTER (WHERE json_extract(value_json, '$.status') = 'completed') as job_completed,
                COUNT(*) FILTER (
                    WHERE json_extract(value_json, '$.status') IS NOT 'completed'
                    AND metric_type = 'job_completion'
                ) as job_failed
            FROM metrics
            {where_clause}
        ...
        for row in rows:
            ...
            if mtype == "llm_call":
                ...
                result["llm_calls"] = {
                    ...
                    "avg_latency_ms": round((row["latency_sum"] or 0) / count, 1) if count else 0,
                ...
                result["tool_calls"] = {
                    ...
                    "avg_duration_ms": round((row["duration_sum"] or 0) / count, 1) if count else 0,
                }
                ...
                result["job_completions"] = {
                    ...
                    "avg_duration_ms": round((row["duration_sum"] or 0) / count, 1) if count else 0,
                    "total_cost_cents": round(row["total_cost_sum"] or 0, 4),
                }
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # ⚡ Bolt: Use window functions and SQL aggregation to calculate SLOs.
            # This avoids loading potentially thousands of job completion rows
            # into Python memory just to calculate a rate and percentile.
