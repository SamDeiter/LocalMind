...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        # for significantly faster processing on large metric sets.
        conn = _connect()
        try:
            where = ""
            params = []
            ...
            query = f"""
                SELECT
                    ...
                    AVG(json_extract(value_json, '$.latency_ms')) FILTER (WHERE metric_type = 'llm_call') as llm_avg_latency,
                    SUM(json_extract(value_json, '$.cost_cents')) FILTER (WHERE metric_type = 'llm_call') as llm_total_cost,
                    ...
                    AVG(json_extract(value_json, '$.duration_ms')) FILTER (WHERE metric_type = 'tool_call') as tool_avg_duration,
                    ...
                    AVG(json_extract(value_json, '$.duration_ms')) FILTER (WHERE metric_type = 'job_completion') as job_avg_duration,
                    SUM(json_extract(value_json, '$.total_cost')) FILTER (WHERE metric_type = 'job_completion') as job_total_cost
                FROM metrics
                {where}
            """
        ...
        return {
            "llm_calls": {
                "total": r["llm_count"] or 0,
                "tokens_in": int(r["llm_tokens_in"] or 0),
                "tokens_out": int(r["llm_tokens_out"] or 0),
                "avg_latency_ms": round(r["llm_avg_latency"] or 0, 1),
                "total_cost_cents": round(r["llm_total_cost"] or 0, 4),
            },
            "tool_calls": {
                "total": r["tool_count"] or 0,
                "success": r["tool_ok"] or 0,
                "error": r["tool_error"] or 0,
                "avg_duration_ms": round(r["tool_avg_duration"] or 0, 1),
            },
            "job_completions": {
                "total": r["job_count"] or 0,
                "completed": r["job_ok"] or 0,
                "failed": r["job_fail"] or 0,
                "avg_duration_ms": round(r["job_avg_duration"] or 0, 1),
                "total_cost_cents": round(r["job_total_cost"] or 0, 4),
            },
        }
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            # ⚡ Bolt: Offload failure rate and P95 latency calculation to SQL.
            # Avoids loading thousands of JSON blobs into Python memory.
            where_clause = ""
            params = []
            if since:
            ...
            row = conn.execute(
                f"""SELECT
                    ...
                    FROM metrics
                    WHERE metric_type = 'job_completion' {where_clause}""",
                params
            ).fetchone()
