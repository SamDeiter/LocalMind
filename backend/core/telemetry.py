...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        conn = _connect()
        try:
            # ⚡ Bolt: Offload JSON parsing and aggregation to SQLite for massive speedup.
            # Reduces latency from O(N) Python parsing to O(N) DB scan with native JSON functions.
            query = """
                SELECT
                    COUNT(*) FILTER (WHERE metric_type = 'llm_call') AS llm_count,
                    SUM(json_extract(value_json, '$.tokens_in')) FILTER (WHERE metric_type = 'llm_call') AS llm_tokens_in,
                    SUM(json_extract(value_json, '$.tokens_out')) FILTER (WHERE metric_type = 'llm_call') AS llm_tokens_out,
                    SUM(json_extract(value_json, '$.latency_ms')) FILTER (WHERE metric_type = 'llm_call') AS llm_latency_sum,
                    SUM(json_extract(value_json, '$.cost_cents')) FILTER (WHERE metric_type = 'llm_call') AS llm_cost_sum,
                    ...
                    SUM(json_extract(value_json, '$.duration_ms')) FILTER (WHERE metric_type = 'tool_call') AS tool_duration_sum,
                    ...
                    SUM(json_extract(value_json, '$.duration_ms')) FILTER (WHERE metric_type = 'job_completion') AS job_duration_sum,
                    SUM(json_extract(value_json, '$.total_cost')) FILTER (WHERE metric_type = 'job_completion') AS job_cost_sum
                FROM metrics
            """
            if since:
                row = conn.execute(query + " WHERE recorded_at >= ?", (since,)).fetchone()
            else:
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            # ⚡ Bolt: Use SQL aggregation and window functions for threshold checks.
            # Avoids O(N) memory and CPU overhead for calculating failure rates and percentiles in Python.
            ...
            # 1. Failure Rate Aggregation
            agg_query = f"""
                SELECT
                    COUNT(*) as total_count,
                    COUNT(*) FILTER (WHERE json_extract(value_json, '$.status') IS NOT 'completed') as failed_count
                FROM metrics
                {where}
            """
            ...
            if row and row["total_count"] and row["total_count"] > 0:
                ...
                # 2. P95 Latency using window functions
                # We use a CTE to rank latencies and pick the 95th percentile row.
                p95_query = f"""
                    WITH ranked_latencies AS (
                        SELECT
                            json_extract(value_json, '$.duration_ms') as duration,
                            ROW_NUMBER() OVER (ORDER BY json_extract(value_json, '$.duration_ms')) as rank,
                    ...
                    WHERE rank = CAST(total_count * 0.95 AS INT) + 1
                    LIMIT 1
                """
                p95_row = conn.execute(p95_query, params).fetchone()
                if p95_row:
                    p95 = p95_row["duration"]
