...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            # ── Job failure rate & latency (SQL optimized) ──────────────
            sql = """
                SELECT
                    COUNT(*) as total,
                    COUNT(*) FILTER (WHERE json_extract(value_json, '$.status') != 'completed') as failed,
                    json_group_array(json_extract(value_json, '$.duration_ms')) as latencies_json
                FROM metrics
            ...
            if row and row["total"] > 0:
                ...
                # ── P95 latency ─────────────────────────────────────────
                try:
                    latencies = sorted(json.loads(row["latencies_json"]))
                    if latencies:
