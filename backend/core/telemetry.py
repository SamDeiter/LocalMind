...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        ⚡ Bolt: Shifted processing to SQLite to avoid O(N) JSON parsing in Python.
        """
        query = """
            SELECT
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # ⚡ Bolt: Optimized data retrieval to avoid full JSON parsing in Python.
            ...
            if counts_row and counts_row["total"] > 0:
                ...
                # Get sorted latencies for P95
                latency_rows = conn.execute(
                    f"SELECT json_extract(value_json, '$.duration_ms') as d FROM metrics {where} ORDER BY d",
                    params
                ).fetchall()

                latencies = [r["d"] for r in latency_rows if r["d"] is not None]
                if latencies:
