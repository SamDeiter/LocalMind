...
def init_telemetry_schema() -> None:
    ...
    conn.executescript("""
        ...
        -- ⚡ Bolt: Composite index to speed up time-filtered aggregations by metric type
...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        # ⚡ Bolt: Use SQL-side aggregation with json_extract for significant speed boost.
        # This reduces data transfer and avoids expensive Python-side loops and JSON parsing.
        sql = """
            SELECT
                metric_type,
                COUNT(*) as count,
                ...
                SUM(CASE WHEN json_extract(value_json, '$.status') = 'ok' THEN 1 ELSE 0 END) as tool_ok,
                SUM(CASE WHEN json_extract(value_json, '$.status') = 'completed' THEN 1 ELSE 0 END) as job_ok
            FROM metrics
            {where}
        ...
        for r in rows:
            ...
            if mtype == "llm_call":
                res["llm_calls"].update({
                    "total": cnt,
                    "tokens_in": int(r["tokens_in"] or 0),
                    "tokens_out": int(r["tokens_out"] or 0),
                    "avg_latency_ms": round((r["latency_sum"] or 0) / cnt, 1) if cnt else 0,
                    "total_cost_cents": round(r["cost_sum"] or 0, 4),
                })
            elif mtype == "tool_call":
                ok = r["tool_ok"] or 0
                res["tool_calls"].update({
                    "total": cnt,
                    "success": ok,
                    "error": cnt - ok,
                    "avg_duration_ms": round((r["duration_sum"] or 0) / cnt, 1) if cnt else 0,
                })
            elif mtype == "job_completion":
                ok = r["job_ok"] or 0
                res["job_completions"].update({
                    "total": cnt,
                    "completed": ok,
                    "failed": cnt - ok,
                    "avg_duration_ms": round((r["duration_sum"] or 0) / cnt, 1) if cnt else 0,
                    "total_cost_cents": round(r["total_cost"] or 0, 4),
                })
