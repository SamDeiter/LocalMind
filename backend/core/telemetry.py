...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        # ⚡ Bolt: Perform aggregation in SQL to avoid fetching all rows into Python.
        # This significantly reduces memory overhead and processing time for large datasets.
        query = """
            SELECT
                COUNT(*) FILTER (WHERE metric_type = 'llm_call') as llm_count,
                TOTAL(json_extract(value_json, '$.tokens_in'))
                    FILTER (WHERE metric_type = 'llm_call') as llm_tokens_in,
                TOTAL(json_extract(value_json, '$.tokens_out'))
                    FILTER (WHERE metric_type = 'llm_call') as llm_tokens_out,
                AVG(json_extract(value_json, '$.latency_ms'))
                    FILTER (WHERE metric_type = 'llm_call') as llm_latency_avg,
                TOTAL(json_extract(value_json, '$.cost_cents'))
                ...
                AVG(json_extract(value_json, '$.duration_ms'))
                    FILTER (WHERE metric_type = 'tool_call') as tool_duration_avg,
                ...
                AVG(json_extract(value_json, '$.duration_ms'))
                    FILTER (WHERE metric_type = 'job_completion') as job_duration_avg,
                TOTAL(json_extract(value_json, '$.total_cost'))
        ...
        if not row:
            return {
                "llm_calls": {
                    "total": 0, "tokens_in": 0, "tokens_out": 0,
                    "avg_latency_ms": 0, "total_cost_cents": 0
                },
                "tool_calls": {
                ...
                "job_completions": {
                    ...
                    "avg_duration_ms": 0, "total_cost_cents": 0
                },
            }
        ...
        return {
            "llm_calls": {
                "total": row["llm_count"],
                "tokens_in": int(row["llm_tokens_in"]),
                "tokens_out": int(row["llm_tokens_out"]),
                "avg_latency_ms": round(row["llm_latency_avg"] or 0, 1),
            ...
            "tool_calls": {
                ...
                "avg_duration_ms": round(row["tool_duration_avg"] or 0, 1),
            },
            "job_completions": {
                "total": row["job_count"],
                "completed": row["job_ok"],
                "failed": row["job_fail"],
                "avg_duration_ms": round(row["job_duration_avg"] or 0, 1),
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            # ── Job failure rate & Latency ──────────────────────────────
            # ⚡ Bolt: Perform calculations in SQL to avoid fetching all job completion rows.
            ...
            if total > 0:
                ...
                # ── P95 latency ─────────────────────────────────────────
                # Use OFFSET to find the 95th percentile value without fetching all rows.
                p95_offset = min(total - 1, int(total * 0.95))
                p95_query = f"""
                    SELECT COALESCE(CAST(json_extract(value_json, '$.duration_ms') AS REAL), 0) as p95
                    FROM metrics {where}
                    ORDER BY p95 ASC
                    LIMIT 1 OFFSET ?
                """
                p95_row = conn.execute(p95_query, params + [p95_offset]).fetchone()
                p95 = p95_row["p95"] if p95_row else 0
