    ...
    def get_metrics_summary(
        ...
        query = f"""
            SELECT
                ...
                COALESCE(SUM(CAST(json_extract(value_json, '$.latency_ms') AS REAL)) FILTER (WHERE metric_type = 'llm_call'), 0.0) AS llm_latency_sum,
                COALESCE(SUM(CAST(json_extract(value_json, '$.cost_cents') AS REAL)) FILTER (WHERE metric_type = 'llm_call'), 0.0) AS llm_cost_sum,
                ...
                COALESCE(SUM(CAST(json_extract(value_json, '$.duration_ms') AS REAL)) FILTER (WHERE metric_type = 'tool_call'), 0.0) AS tool_duration_sum,
                ...
                COALESCE(SUM(CAST(json_extract(value_json, '$.duration_ms') AS REAL)) FILTER (WHERE metric_type = 'job_completion'), 0.0) AS job_duration_sum,
                COALESCE(SUM(CAST(json_extract(value_json, '$.total_cost') AS REAL)) FILTER (WHERE metric_type = 'job_completion'), 0.0) AS job_cost_sum
            FROM metrics {where}
        """
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if stats and stats["total"] > 0:
                ...
                # ── P95 latency ─────────────────────────────────────────
                # Calculate P95 offset: e.g. for 100 rows, offset 94 is the 95th element
