...
def init_telemetry_schema() -> None:
    ...
    conn.executescript("""
        ...
        -- ⚡ Bolt: Composite index to optimize get_metrics_summary and threshold checks.
        -- Allows SQLite to filter by time and type in a single scan.
...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        conn = _connect()
        try:
            # ⚡ Bolt: Aggregated in SQL for maximum performance.
            # Using FILTER and json_extract avoids full value_json parsing in Python.
            # TOTAL() returns 0.0 instead of NULL when no rows match.
            query = """
                SELECT
        ...
        if not row:
            # Aggregate queries on empty tables return one row of zeros/nulls in SQLite
            return {
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            # ── Job failure rate & P95 ──────────────────────────────────
            # ⚡ Bolt: Offload failure rate calculation to SQL.
            # Retrieve only duration_ms for P95 to avoid full JSON parsing.
            ...
            if stats and stats["total"] > 0:
                ...
                # ── P95 latency ─────────────────────────────────────────
                # Only select the duration_ms field from JSON.
                rows = conn.execute(
                    f"SELECT json_extract(value_json, '$.duration_ms') as d FROM metrics {where}",
                    params
                ).fetchall()
                latencies = sorted(r["d"] for r in rows if r["d"] is not None)
