...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        # ⚡ Bolt: Use SQL aggregations with json_extract and FILTER to compute metrics
        # on the database side. This is significantly faster than fetching thousands
        # of JSON strings and parsing them in a Python loop.
        conn = _connect()
        try:
            ...
            query = f"""
                SELECT
                    ...
                    TOTAL(json_extract(value_json, '$.latency_ms'))
                    ...
                    COUNT(*) FILTER (WHERE metric_type = 'tool_call' AND
                                     json_extract(value_json, '$.status') = 'ok') as tool_ok,
                    COUNT(*) FILTER (WHERE metric_type = 'tool_call' AND
                                     json_extract(value_json, '$.status') IS NOT 'ok') as tool_error,
                    TOTAL(json_extract(value_json, '$.duration_ms'))
                        FILTER (WHERE metric_type = 'tool_call') as tool_duration_sum,

                    COUNT(*) FILTER (WHERE metric_type = 'job_completion') as job_count,
                    COUNT(*) FILTER (WHERE metric_type = 'job_completion' AND
                                     json_extract(value_json, '$.status') = 'completed') as job_ok,
                    COUNT(*) FILTER (WHERE metric_type = 'job_completion' AND
                                     json_extract(value_json, '$.status') IS NOT 'completed') as job_fail,
                    TOTAL(json_extract(value_json, '$.duration_ms'))
        ...
        return {
            "llm_calls": {
                "total": res["llm_count"],
                "tokens_in": int(res["llm_tokens_in"]),
                "tokens_out": int(res["llm_tokens_out"]),
                "avg_latency_ms": round(res["llm_latency_sum"] / res["llm_count"], 1) if res["llm_count"] else 0,
                "total_cost_cents": round(res["llm_cost_sum"], 4),
            },
            "tool_calls": {
                "total": res["tool_count"],
                "success": res["tool_ok"],
                "error": res["tool_error"],
                "avg_duration_ms": round(res["tool_duration_sum"] / res["tool_count"], 1) if res["tool_count"] else 0,
            },
            "job_completions": {
                "total": res["job_count"],
                "completed": res["job_ok"],
                "failed": res["job_fail"],
                "avg_duration_ms": round(res["job_duration_sum"] / res["job_count"], 1) if res["job_count"] else 0,
                "total_cost_cents": round(res["job_cost_sum"], 4),
            },
        }
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        """
        # ⚡ Bolt: Use SQL aggregations and window functions for threshold checks.
        # This avoids the overhead of fetching and parsing thousands of JSON rows.
        ...
        try:
            ...
            # Combined query for throughput metrics
            query = f"""
                WITH job_metrics AS (
                    SELECT
                ...
                SELECT
                    ...
                    (SELECT duration_ms FROM ranked_latencies
                     WHERE row_num = CAST(total_count * 0.95 AS INT) + 1 LIMIT 1) as p95
                FROM job_metrics
            """
            res = conn.execute(query, params).fetchone()
