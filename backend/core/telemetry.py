...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        # ⚡ Bolt: Use SQL aggregation and json_extract to minimize Python-side iteration.
        # Offloading work to the DB engine is faster and more memory-efficient.
        sql = """
            SELECT
        ...
        # Use COALESCE logic: SUM returns NULL on empty sets
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            # ⚡ Bolt: Use SQL aggregation for failure rate and targeted offset for P95.
            # Avoids loading thousands of records into Python memory.
            ...
            stats_row = conn.execute(
                ...
                FROM metrics {where}""", params
            ).fetchone()

            if stats_row and stats_row["total"] > 0:
                ...
                # 2. P95 latency via targeted LIMIT/OFFSET
                p95_offset = max(0, int(total * 0.95) - 1)
                p95_row = conn.execute(
                    f"""SELECT json_extract(value_json, '$.duration_ms') as p95
                    FROM metrics {where}
                    ORDER BY p95 ASC
                    LIMIT 1 OFFSET ?""", params + [p95_offset]
                ).fetchone()
