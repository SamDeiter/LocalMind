...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        # ⚡ Bolt: Optimize by running a single-query SQL aggregation using SQLite aggregate functions,
        # json_extract(), and FILTER clauses instead of parsing every row in a Python loop.
        conn = _connect()
        try:
            where_clause = ""
            params = []
            ...
            query = f"""
                SELECT
                    ...
                    COALESCE(SUM(CAST(json_extract(value_json, '$.tokens_in') AS INTEGER))
                        FILTER (WHERE metric_type = 'llm_call'), 0) AS llm_tokens_in,
                    COALESCE(SUM(CAST(json_extract(value_json, '$.tokens_out') AS INTEGER))
                        FILTER (WHERE metric_type = 'llm_call'), 0) AS llm_tokens_out,
                    COALESCE(SUM(CAST(json_extract(value_json, '$.latency_ms') AS REAL))
                        FILTER (WHERE metric_type = 'llm_call'), 0.0) AS llm_latency_sum,
                    COALESCE(SUM(CAST(json_extract(value_json, '$.cost_cents') AS REAL))
                        FILTER (WHERE metric_type = 'llm_call'), 0.0) AS llm_cost_sum,
                    ...
                    COUNT(*) FILTER (WHERE metric_type = 'tool_call'
                        AND COALESCE(json_extract(value_json, '$.status'), '') != 'ok') AS tool_error,
                    COALESCE(SUM(CAST(json_extract(value_json, '$.duration_ms') AS REAL))
                        FILTER (WHERE metric_type = 'tool_call'), 0.0) AS tool_duration_sum,

                    -- job_completion
                    COUNT(*) FILTER (WHERE metric_type = 'job_completion') AS job_count,
                    COUNT(*) FILTER (WHERE metric_type = 'job_completion'
                        AND json_extract(value_json, '$.status') = 'completed') AS job_completed,
                    COUNT(*) FILTER (WHERE metric_type = 'job_completion'
                        AND COALESCE(json_extract(value_json, '$.status'), '') != 'completed') AS job_failed,
                    COALESCE(SUM(CAST(json_extract(value_json, '$.duration_ms') AS REAL))
                        FILTER (WHERE metric_type = 'job_completion'), 0.0) AS job_duration_sum,
                    COALESCE(SUM(CAST(json_extract(value_json, '$.total_cost') AS REAL))
                        FILTER (WHERE metric_type = 'job_completion'), 0.0) AS job_cost_sum
                FROM metrics
                {where_clause}
            """
        ...
        if row is None:
            return {
                "llm_calls": {
                    "total": 0, "tokens_in": 0, "tokens_out": 0, "avg_latency_ms": 0, "total_cost_cents": 0.0
                },
                "tool_calls": {
                    "total": 0, "success": 0, "error": 0, "avg_duration_ms": 0
                },
                "job_completions": {
                    "total": 0, "completed": 0, "failed": 0, "avg_duration_ms": 0, "total_cost_cents": 0.0
                },
            }
        ...
        return {
            "llm_calls": {
                ...
                "avg_latency_ms": round((row["llm_latency_sum"] or 0.0) / llm_count, 1) if llm_count else 0,
                "total_cost_cents": round(row["llm_cost_sum"] or 0.0, 4),
            },
            "tool_calls": {
                "total": tool_count,
                "success": row["tool_ok"] or 0,
                "error": row["tool_error"] or 0,
                "avg_duration_ms": round((row["tool_duration_sum"] or 0.0) / tool_count, 1) if tool_count else 0,
            },
            "job_completions": {
                "total": job_count,
                "completed": row["job_completed"] or 0,
                "failed": row["job_failed"] or 0,
                "avg_duration_ms": round((row["job_duration_sum"] or 0.0) / job_count, 1) if job_count else 0,
                "total_cost_cents": round(row["job_cost_sum"] or 0.0, 4),
            },
        }
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        """
        # ⚡ Bolt: Optimize by replacing manual Python loop parsing of status and sorting of
        # latencies with fast, direct SQL-level counting, filtering, and sorted limit/offset queries.
        ...
        try:
            ...
            query = f"""
                SELECT
                    ...
                    COALESCE(SUM(CASE WHEN
                        COALESCE(json_extract(value_json, '$.status'), '') != 'completed'
            ...
            if row and row["total"] > 0:
                ...
                # ── P95 latency ─────────────────────────────────────────
                # Calculate the 95th percentile index in Python matching exact sorting index
                p95_idx = int(total * 0.95)
                offset = min(p95_idx, total - 1)
                ...
                p95_query = f"""
                    ...
                    LIMIT 1 OFFSET ?
                """
                p95_row = conn.execute(p95_query, params + [offset]).fetchone()
                if p95_row:
                    p95 = p95_row["duration"] or 0.0
