...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        try:
            ...
            # ⚡ Bolt: Offload aggregation and JSON parsing to SQLite for ~50% speedup.
            # Use COALESCE(SUM(...), 0) inside FILTER if possible, or wrap the whole expression.
            # SQLite requires FILTER to be attached directly to the aggregate function.
            query = f"""
            SELECT
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # ⚡ Bolt: Use window functions to calculate failure rate and p95 in one pass.
            where_since = ""
            params: list[Any] = []
            if since:
                where_since = "AND recorded_at >= ?"
                params.append(since)

            query = f"""
                SELECT total, failed, p95
                FROM (
                    SELECT
                        COUNT(*) OVER () as total,
                        COUNT(*) FILTER (WHERE json_extract(value_json, '$.status') IS NOT 'completed') OVER () as failed,
                        COALESCE(CAST(json_extract(value_json, '$.duration_ms') AS REAL), 0) as p95,
                        ROW_NUMBER() OVER (ORDER BY COALESCE(CAST(json_extract(value_json, '$.duration_ms') AS REAL), 0) ASC) as row_num
                    FROM metrics
                    WHERE metric_type = 'job_completion' {where_since}
                )
                WHERE row_num = MAX(1, CAST(total * 0.95 AS INT))
            """
            row = conn.execute(query, params).fetchone()
