...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        try:
            ...
            query = """
                SELECT
                    ...
                    COALESCE(SUM(json_extract(value_json, '$.tokens_in')) FILTER (WHERE metric_type = 'llm_call'), 0) as llm_tokens_in,
                    COALESCE(SUM(json_extract(value_json, '$.tokens_out')) FILTER (WHERE metric_type = 'llm_call'), 0) as llm_tokens_out,
                    COALESCE(SUM(json_extract(value_json, '$.latency_ms')) FILTER (WHERE metric_type = 'llm_call'), 0) as llm_latency_sum,
                    COALESCE(SUM(json_extract(value_json, '$.cost_cents')) FILTER (WHERE metric_type = 'llm_call'), 0) as llm_cost_sum,
                    ...
                    COALESCE(SUM(json_extract(value_json, '$.duration_ms')) FILTER (WHERE metric_type = 'tool_call'), 0) as tool_duration_sum,
                    ...
                    COALESCE(SUM(json_extract(value_json, '$.duration_ms')) FILTER (WHERE metric_type = 'job_completion'), 0) as job_duration_sum,
                    COALESCE(SUM(json_extract(value_json, '$.total_cost')) FILTER (WHERE metric_type = 'job_completion'), 0) as job_cost_sum
                FROM metrics
            """
            if since:
        ...
        return {
            "llm_calls": {
                "total": res.get("llm_count", 0),
                "tokens_in": res.get("llm_tokens_in", 0),
                "tokens_out": res.get("llm_tokens_out", 0),
                "avg_latency_ms": round(res.get("llm_latency_sum", 0) / res["llm_count"], 1) if res.get("llm_count") else 0,
                "total_cost_cents": round(res.get("llm_cost_sum", 0), 4),
            },
            "tool_calls": {
                "total": res.get("tool_count", 0),
                "success": res.get("tool_ok", 0),
                "error": res.get("tool_error", 0),
                "avg_duration_ms": round(res.get("tool_duration_sum", 0) / res["tool_count"], 1) if res.get("tool_count") else 0,
            },
            "job_completions": {
                "total": res.get("job_count", 0),
                "completed": res.get("job_ok", 0),
                "failed": res.get("job_fail", 0),
                "avg_duration_ms": round(res.get("job_duration_sum", 0) / res["job_count"], 1) if res.get("job_count") else 0,
                "total_cost_cents": round(res.get("job_cost_sum", 0), 4),
            },
        }
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            # ⚡ Bolt: Offload failure rate and P95 calculation to SQL.
            # Avoids loading thousands of job completion rows into Python.
            ...
            # 2. P95 Latency (using a subquery for offset to find the 95th percentile)
            if row and row["total"] > 0:
                p95_offset = max(0, int(row["total"] * 0.95) - 1)
                p95_row = conn.execute(f"""
                    SELECT CAST(json_extract(value_json, '$.duration_ms') AS REAL) as p95
                    FROM metrics {where_clause}
                    ORDER BY p95 ASC
