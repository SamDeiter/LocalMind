...
class MetricsCollector:
    ...
    def get_metrics_summary(self, since: Optional[str] = None) -> dict[str, Any]:
        """Produce an aggregated metrics summary using server-side SQL aggregation."""
        conn = _connect()
        try:
            where_clause = "WHERE recorded_at >= ?" if since else ""
            params = [since] if since else []

            # ⚡ Bolt: Use SQL aggregation and json_extract to avoid fetching all rows.
            # TOTAL() is used instead of SUM() because it returns 0.0 instead of NULL when no rows match.
            query = f"""
                SELECT
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            where_base = "WHERE metric_type = 'job_completion'"
            params: list[Any] = []
            if since:
                where_base += " AND recorded_at >= ?"
                params.append(since)

            # ⚡ Bolt: Calculate failure rate and P95 latency in SQL.
            # Uses a CTE and Window Functions for better performance and compatibility with older SQLite versions.
            stats_query = f"""
                WITH job_data AS (
                    SELECT
                ...
                SELECT
                    ...
                    (
                        SELECT duration_ms FROM (
                            SELECT duration_ms, ROW_NUMBER() OVER (ORDER BY duration_ms ASC) as row_num, COUNT(*) OVER () as cnt
                            FROM job_data
                        ) WHERE row_num = MAX(1, CAST(cnt * 0.95 AS INT)) LIMIT 1
                    ) as p95
                FROM job_data
            """
            r = conn.execute(stats_query, params).fetchone()
