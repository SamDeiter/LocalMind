...
class MetricsCollector:
    ...
    def get_metrics_summary(
        ...
        """
        Produce an aggregated metrics summary using SQL-native aggregation.
        ...
        """
        # ⚡ Bolt: Offload aggregation to SQLite using FILTER and json_extract
        # This avoids fetching thousands of rows and parsing JSON in Python.
        query = """
            SELECT
                -- LLM Calls
                COUNT(*) FILTER (WHERE metric_type = 'llm_call') AS llm_total,
                SUM(CAST(json_extract(value_json, '$.tokens_in') AS INTEGER)) FILTER (WHERE metric_type = 'llm_call') AS llm_tokens_in,
                SUM(CAST(json_extract(value_json, '$.tokens_out') AS INTEGER)) FILTER (WHERE metric_type = 'llm_call') AS llm_tokens_out,
                SUM(CAST(json_extract(value_json, '$.latency_ms') AS REAL)) FILTER (WHERE metric_type = 'llm_call') AS llm_latency_sum,
                SUM(CAST(json_extract(value_json, '$.cost_cents') AS REAL)) FILTER (WHERE metric_type = 'llm_call') AS llm_cost_sum,

                -- Tool Calls
                COUNT(*) FILTER (WHERE metric_type = 'tool_call') AS tool_total,
                COUNT(*) FILTER (WHERE metric_type = 'tool_call' AND json_extract(value_json, '$.status') = 'ok') AS tool_success,
                COUNT(*) FILTER (WHERE metric_type = 'tool_call' AND json_extract(value_json, '$.status') IS NOT 'ok') AS tool_error,
                SUM(CAST(json_extract(value_json, '$.duration_ms') AS REAL)) FILTER (WHERE metric_type = 'tool_call') AS tool_duration_sum,

                -- Job Completions
                COUNT(*) FILTER (WHERE metric_type = 'job_completion') AS job_total,
                COUNT(*) FILTER (WHERE metric_type = 'job_completion' AND json_extract(value_json, '$.status') = 'completed') AS job_completed,
                COUNT(*) FILTER (WHERE metric_type = 'job_completion' AND json_extract(value_json, '$.status') IS NOT 'completed') AS job_failed,
                SUM(CAST(json_extract(value_json, '$.duration_ms') AS REAL)) FILTER (WHERE metric_type = 'job_completion') AS job_duration_sum,
                SUM(CAST(json_extract(value_json, '$.total_cost') AS REAL)) FILTER (WHERE metric_type = 'job_completion') AS job_cost_sum
            FROM metrics
        """
        params = []
        if since:
        ...
        # SQL aggregate functions on empty sets or NULL results return NULL; normalize to 0
        r = {k: (row[k] if row[k] is not None else 0) for k in row.keys()}
        ...
        return {
            "llm_calls": {
                ...
                "avg_latency_ms": round(r["llm_latency_sum"] / r["llm_total"], 1) if r["llm_total"] else 0,
            ...
            "tool_calls": {
                ...
                "error": r["tool_error"],
                "avg_duration_ms": round(r["tool_duration_sum"] / r["tool_total"], 1) if r["tool_total"] else 0,
            },
            "job_completions": {
                "total": r["job_total"],
                "completed": r["job_completed"],
                "failed": r["job_failed"],
                "avg_duration_ms": round(r["job_duration_sum"] / r["job_total"], 1) if r["job_total"] else 0,
...
class AlertManager:
    ...
    def check_thresholds(self, since: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        conn = _connect()
        try:
            # ⚡ Bolt: Offload failure rate and P95 latency to SQL using CTEs and window functions.
            # This avoids fetching all rows and sorting/calculating in Python.
            ...
            query = f"""
                WITH job_metrics AS (
                    ...
                    FROM metrics
                    WHERE metric_type = 'job_completion'
                    {since_clause}
                ),
                stats AS (
                    SELECT
                        COUNT(*) as total_count,
                        COUNT(*) FILTER (WHERE status IS NOT 'completed') as failed_count
                    FROM job_metrics
                ),
                percentiles AS (
                    SELECT
                ...
                SELECT
                    s.total_count,
                    s.failed_count,
                    (SELECT duration_ms FROM percentiles WHERE row_num = MAX(1, CAST(total_rows * 0.95 AS INTEGER))) as p95_latency
                FROM stats s
            """
